...
export default function ValetDashboard() {
  ...
  useEffect(() => {
    async function fetchLocation() {
      ...
      const locationId = user.locationId?.$oid || user.locationId._id || user.locationId;
      try {
  ...
  useEffect(() => {
    ...
    socket.on("ticket:created", (ticket) => setTickets((prev) => [...prev, ticket]));
  ...
  const handleUpdate = async (ticketId, updates) => {
    try {
      ...
      setTickets((prev) =>
        prev.map((t) => (t._id === ticketId || t.ticketId === ticketId ? res.data.ticket : t))
      );
    } catch (err) {
  ...
  return (
    <div className="p-6">
      ...
      <table className="w-full border">{tickets.map(renderTicketRow)}</table>
    </div>
  );
}